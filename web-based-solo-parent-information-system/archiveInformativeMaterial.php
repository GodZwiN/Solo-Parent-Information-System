...
if (isset($_POST['uploadInformativeMaterial'])) {
    ...
    if (in_array($fileActualExt, $allowed)) {
        if (isInformativeMaterialExist($pdf)){
            header("Location: archiveInformativeMaterial.php?informativeMaterialExist");
        } else {
            $pdf_type = $_FILES['pdf']['type'];
            $pdf_size = $_FILES['pdf']['size'];
            $pdf_tem_loc= $_FILES['pdf']['tmp_name'];
        ...
        header("Location: archiveInformativeMaterial.php?invalidFileFormat");
    }
}
?>
            ...
            <div class="row">
                ...
                if(isset($_GET['informativeMaterialExist'])){
                    echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                        Informative material already exist, upload failed.
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>';
                }
                if(isset($_GET['invalidFileFormat'])){
                    echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                        Informative material upload failed. Only PDF, WORD, EXCEL files are allowed. 
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>';
                }
                ?>
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo "<tr>
                                    ...
                                    <td>
                                        <div class='row g-1'>
                                        <div class='col'>
                                        <form action='viewMaterial.php' target='_blank' method='GET' class='d-grid'>
                                        ...
                                        <div class='modal-dialog modal-dialog-centered'>
                                            <div class='modal-content'>
                                            <div class='modal-header'>
                                                <h1 class='modal-title fs-md fw-bolder' id='passwordConfirmation'>Confirm Deletion</h1>
                                                <button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button>
                                            </div>
                                            <form action='deleteMaterial.php' method='GET' id='delete".$row['InformativeMaterialID']."' class='d-inline-block'>
                                                <div class='modal-body text-center'>
                                                Delete <span class='fw-bold'>" . $row['InformativeMaterial'] . '?' . " </span>
                                                    <div class='row'>
                                                        <div class='col'>
