...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['addSoloParent'])) {
    ...
    if (date('Y', strtotime($birthdate)) >= date('Y') - 14 || date('Y', strtotime($birthdate)) - (date('Y') - 14) >= 60){
                ...
                if (!$isChildAlreadyExist){
                    if (!(mysqli_query($connection, addSoloParent($controlNumber, $firstName, $middleName, $lastName, $sex, $birthdate, $barangay, $streetAddress, $occupation, $phoneNumber, $natureOfSoloParent, $pwdStatus, $fourPsStatus)))) {
                        ...
                        if ($isAllChildRecordAdded) {
                            $soloParentFullName = strtoupper($firstName) . " " . strtoupper($middleName) . " " . strtoupper($lastName);
                            addAddSoloParentAudit($_SESSION['loggedAccount']['FirstName'],$_SESSION['loggedAccount']['LastName'],$soloParentFullName, $controlNumber, $_SESSION['loggedAccount']['Username']);
                    ...
                    <form action="#" method="POST">
                        ...
                        <div class="row g-2">
                            <div class="col-lg-3 mb-3">
                                ...
                                <input type="text" autocomplete="off" readonly class="form-control" id="controlNumber" name="controlNumber" value="<?='BNY ' . date('y') . '-' . date('m') . '-' . getNumberInQueue()?>" required>
                                <span class="error-message text-danger"><?= $controlNumberErrorMessage ?></span>
                            </div>
                            <div class="col-lg-3 mb-3">
                                <label for="firstName" class="form-label">First Name</label>
                                <input type="text" autocomplete="off" class="form-control" id="firstName" name="firstName" placeholder="Juan" value="<?= $firstName?>" required onblur="">
                                <span class="error-message text-danger" id="firstNameErrorMessage"><?= $firstNameErrorMessage ?></span>
                            </div>
                            <div class="col-lg-3 mb-3">
                                <label for="middleName" class="form-label">Middle Name</label>
                                <input type="text" autocomplete="off" class="form-control" id="middleName" name="middleName" placeholder="San Juan" value="<?= $middleName?>">
                                <span class="error-message text-danger " id="middleNameErrorMessage"><?= $middleNameErrorMessage ?></span>
                            </div>
                            <div class="col-lg-3 mb-3">
                                <label for="lastName" class="form-label">Last Name</label>
                                <input type="text" autocomplete="off" class="form-control" id="lastName" name="lastName" placeholder="Dela Cruz" value="<?= $lastName?>" required>
                                <span class="error-message text-danger " id="lastNameErrorMessage"><?= $lastNameErrorMessage ?></span>
                            </div>
                        </div>
                        <div class="row g-2">
                            ...
                            <div class="col-lg-3 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" autocomplete="off" class="form-control" id="phoneNumber" name="phoneNumber" placeholder="XXXXXXXXXX" value="<?= $phoneNumber?>" required>
                                </div>
                                <span class="error-message text-danger " id="phoneNumberErrorMessage"><?= $phoneNumberErrorMessage ?></span>
                            </div>
                            <div class="col mb-3">
                                <label for="occupation" class="form-label">Occupation</label>
                                <input type="text" autocomplete="off" class="form-control" id="occupation" name="occupation" placeholder="Ex. Production Operator" value="<?= $occupation?>" required>
                                <span class="error-message text-danger " id="occupationErrorMessage"><?= $occupationErrorMessage ?></span>
                            </div>
                        </div>
                        <div class="row g-2">
                            <div class="col-lg-3 mb-3">
                            ...
                            <div class="col mb-3">
                                ...
                                <input type="text" autocomplete="off" class="form-control" id="streetAddress" name="streetAddress" placeholder="Block # Lot # Sample St. Sample Village" value="<?= $streetAddress?>" required>
                        ...
                        <div class="row g-2">
                            ...
                            <div class="col-lg-6 mb-3">
                                ...
                                <input type="number" class="form-control" id="numberOfChild" name="numberOfChild" min="1" max="99" value="<?= $numberOfChild?>" onchange="maintainMaxLimit();" onblur="generateChildFields()" required>
                            </div>
                        </div>
                        <div class="row mt-2">
                            ...
                            <div id="childFields">
                                <div class="row g-2 mt-2">
                                    ...
                                    <div class="col-lg-4 mb-3">
                                        ...
                                        <input type="text" autocomplete="off" class="form-control" id="child1FirstName" name="child1FirstName" placeholder="Pedro" required>
                                    </div>
                                    <div class="col-lg-4 mb-3">
                                        <label for="child1MiddleName" class="form-label">Middle Name</label>
                                        <input type="text" autocomplete="off" class="form-control" id="child1MiddleName" name="child1MiddleName" placeholder="San Juan" required>
                                    </div>
                                    <div class="col-lg-4 mb-3">
                                        <label for="child1LastName" class="form-label">Last Name</label>
                                        <input type="text" autocomplete="off" class="form-control" id="child1LastName" name="child1LastName" placeholder="Dela Cruz" required>
                                    </div>
                                </div>
                                <div class="row g-2">
                                    <div class="col-lg-4 mb-3">
...
<script>
    document.getElementById('firstName').onkeyup = function () {
        if (allLetter(document.getElementById('firstName'))) {
            ...
            document.getElementById('firstNameErrorMessage').innerHTML = 'Invalid first name';
        }
    };

    document.getElementById('middleName').onkeyup = function () {
        if (allLetter(document.getElementById('middleName'))) {
            document.getElementById('middleName').classList.remove('is-invalid');
            document.getElementById('middleName').classList.add('is-valid');
            document.getElementById('middleNameErrorMessage').innerHTML = '';
        } else {
            document.getElementById('middleName').classList.remove('is-valid');
            document.getElementById('middleName').classList.add('is-invalid');
            document.getElementById('middleNameErrorMessage').innerHTML = 'Invalid middle name';
        }
    ...
    document.getElementById('lastName').onkeyup = function () {
        if (allLetter(document.getElementById('lastName'))) {
            ...
            document.getElementById('lastNameErrorMessage').innerHTML = 'Invalid last name';
        }
    };

    document.getElementById('phoneNumber').onkeyup = function () {
        if (document.getElementById('phoneNumber').value <= 9999999999 && document.getElementById('phoneNumber').value != '') {
            ...
            document.getElementById('phoneNumberErrorMessage').innerHTML = 'Invalid phone number';
        }
    };

    document.getElementById('phoneNumber').onblur = function () {
        if (document.getElementById('phoneNumber').value < 999999999) {
            document.getElementById('phoneNumber').classList.remove('is-valid');
            document.getElementById('phoneNumber').classList.add('is-invalid');
            document.getElementById('phoneNumberErrorMessage').innerHTML = 'Invalid phone number';
        }
    ...
    document.getElementById('occupation').onkeyup = function () {
        if (allLetter(document.getElementById('occupation'))) {
            document.getElementById('occupation').classList.remove('is-invalid');
            document.getElementById('occupation').classList.add('is-valid');
            document.getElementById('occupationErrorMessage').innerHTML = '';
        } else {
            document.getElementById('occupation').classList.remove('is-valid');
            document.getElementById('occupation').classList.add('is-invalid');
            document.getElementById('occupationErrorMessage').innerHTML = 'Invalid occupation';
        }
    };
