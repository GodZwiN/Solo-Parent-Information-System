...
function maintainMaxLimit() {
	let numberOfChild = document.getElementById('numberOfChild').value;

	if (numberOfChild > 99) {
		document.getElementById('numberOfChild').value = 99;
	}

	if (numberOfChild < 1) {
		document.getElementById('numberOfChild').value = 1;
	}
}
...
function generateChildFields() {
	...
	for (let i = 0; i < numberOfChild; i++) {
		...
		middleNameInput.autocomplete = 'off';
		...
		firstNameInput.onkeyup = function () {
			if (allLetter(firstNameInput)) {
				...
				firstNameErrorMessage.innerHTML = 'Invalid first name';
			}
		};
		middleNameInput.onkeyup = function () {
			if (allLetter(middleNameInput)) {
				middleNameInput.classList.remove('is-invalid');
				middleNameInput.classList.add('is-valid');
				middleNameErrorMessage.innerHTML = '';
			} else {
				middleNameInput.classList.remove('is-valid');
				middleNameInput.classList.add('is-invalid');
				middleNameErrorMessage.innerHTML = 'Invalid middle name';
			}
		};

		middleNameInput.onblur = function () {
		...
		lastNameInput.onkeyup = function () {
			if (allLetter(lastNameInput)) {
				...
				lastNameErrorMessage.innerHTML = 'Invalid last name';
			}
		};

		childRelationshipSelection.onchange = function () {
			if (childRelationshipSelection.value == '2') {
