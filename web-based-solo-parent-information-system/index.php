...
if (!isSomeoneIsLoggedIn()) {
    ...
    if (!isset($_COOKIE['currentMonth'])) {
        setCurrentMonth();
    }

    if (!isset($_COOKIE['lastDayOpened'])) {
        setLastDayOpened(date('d'));
    }

    if (!isset($_COOKIE['numberInQueue']) || $_COOKIE['currentMonth'] < date('m')){
        setNumberInQueue();
    }
