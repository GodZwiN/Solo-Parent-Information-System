<?php 
    ...
    function generateSoloParentRecords() {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $buttons = "<div class='row g-1'>
                            <div class='col-lg-12 d-grid'>
                                <form  action='editOrViewSoloParentRecord.php' class='d-grid' method='GET'>
                            ...
                            <div class='col-lg-12 d-grid'>
                                <form  action='renewSoloParentRecord.php' method='GET' class='d-grid'>
                                    <input type='hidden' name='SoloParentID' value='" . $row['SoloParentID'] ."'>
            ...
            if ($row['MembershipStatus'] === "ACTIVE") {
                $buttons = "<div class='row g-1'>
                                <div class='col-lg-12 d-grid'>
                                    <form  action='editOrViewSoloParentRecord.php' class='d-grid' method='GET'>
                                        <input type='hidden' name='SoloParentID' value='" . $row['SoloParentID'] ."'>
                                        <button class='btn btn-danger fw-bold fs-xs' type='submit'>View/Update</button>
                                    </form>
                                </div> 
                            </div>";
    ...
    function updateSoloParentControlNumber($soloParentID) {
        global $connection;
        $currentAvailableControlNumber = 'BNY ' . date('y') . '-' . date('m') . '-' . getNumberInQueue();

        $sql = "UPDATE soloparents SET ControlNumber = '$currentAvailableControlNumber' WHERE SoloParentID = $soloParentID";
    ...
    function generateSoloParentRecordsForIDGeneration() {
        ...
        $sql = "SELECT SoloParentID, ControlNumber, LastName, FirstName, MiddleName, Birthdate, Barangay, NatureOfSoloParent, Sex, PWDStatus, MembershipStatus, PhoneNumber 
                FROM soloparents
                INNER JOIN barangays
                    ON soloparents.BarangayID = barangays.BarangayID
                INNER JOIN naturesofsoloparent
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $diff = date_diff(date_create($dateOfBirth), date_create($today));
                ...
                <td>" .$row['MiddleName'] . "</td>
                <td>" .$diff->format('%y') . "</td>
                <td>" .strtoupper(date_format(date_create($row['Birthdate']), "M d, Y")) . "</td>
                <td>".getSex($row['Sex'])."</td>
                <td>" .$row['Barangay'] . "</td>
                    ...
                    <div class='row g-1'>
                        <div class='col-lg-12 d-grid'>
                            <form  action='generateID.php' class='mb-1' method='GET'>
                                <input type='hidden' name='SoloParentID' value='" . $row['SoloParentID'] ."'>
                                <button class='btn btn-danger fw-bold fs-xs' type='submit'>Generate</button>
                            </form>
                        </div> 
                    </div>
                </td>
    ...
    function setLastDayOpened($date) {
        setcookie('lastDayOpened', $date, time() + (86400 * 3652));
    }

    function setNumberInQueue() {
        setcookie('numberInQueue', 1, time() + (86400 * 3652));
    }

    function setCurrentMonth() {
        setcookie('currentMonth', date('m'), time() + (86400 * 3652));
    }

    function incrementNumberInQueue() {
        $nextQueueNumber = intval($_COOKIE['numberInQueue']) + 1;
        setcookie('numberInQueue', $nextQueueNumber, time() + (86400 * 3652));
    }
    function getNumberInQueue() {
        return str_pad($_COOKIE['numberInQueue'], 4, '0', STR_PAD_LEFT);
    }
    ...
    function getPopulationPercentage ($population, $totalPopulation) {
        return round(($population/$totalPopulation*100),2) . '%';
    } 
    ...
    function getOrdinal($value) {
        ...
        if ($value[strlen($value) - 1] === '2') {
            return 'nd';
        }
        if ($value[strlen($value) - 1] === '3') {
            return 'rd';
        }
        return 'th';
    }
    ...
    function addSignUpAudit($firstName, $lastName, $username) {
        global $connection;

        $time = date('d M Y | h:i A');
        $details = $firstName . " " . $lastName . " created an account.";
        $user = $username;

        $sql = "INSERT INTO `audittrail` (`TrailID`, `Time`, `Action`, `Details`, `User`) VALUES (NULL, '$time', 'Account Creation', '$details', '$user')";

        $result = mysqli_query($connection, $sql);
    }

    function addActivateOrDeactivateEmployeeAccountAudit($employeeAccountID, $accountStatus) {
        global $connection;

        $sql = "SELECT * FROM accounts WHERE AccountID = $employeeAccountID";
        $result = mysqli_query($connection, $sql);
        ...
        $time = date('d M Y | h:i A');
        $action = "Administrator " . $action ." the account of " . $account['FirstName'] . ' ' . $account['LastName'] . " (" . $account['Username'] . ").";
        $user = 'Administrator';

        $sql = "INSERT INTO `audittrail` (`TrailID`, `Time`, `Action`, `Details`, `User`) VALUES (NULL, '$time', 'Activation/Deactivation of Account', '$action', '$user')";

        $result = mysqli_query($connection, $sql);
    }

    function addAccountUpdateAudit($firstName, $lastName, $username) {
        global $connection;

        $time = date('d M Y | h:i A');
        $action = $firstName . ' ' . $lastName . " updated his/her account details.";
        $user = $username;

        $sql = "INSERT INTO `audittrail` (`TrailID`, `Time`, `Action`, `Details`, `User`) VALUES (NULL, '$time', 'Account Update', '$action', '$user')";

        $result = mysqli_query($connection, $sql);
    }

    function addAddSoloParentAudit($userFirstName, $userLastName, $soloParentFullName, $soloParentControlNumber, $username) {
        global $connection;

        $time = date('d M Y | h:i A');
        $action = $userFirstName . ' ' . $userLastName . " created a record for " . $soloParentFullName . " (" . $soloParentControlNumber . ").";
        $user = $username;

        $sql = "INSERT INTO `audittrail` (`TrailID`, `Time`, `Action`, `Details`, `User`) VALUES (NULL, '$time', 'Addition of Solo Parent Record', '$action', '$user')";

        $result = mysqli_query($connection, $sql);
    }

    function addEditSoloParentAudit($userFirstName, $userLastName, $soloParentFullName, $soloParentControlNumber, $username) {
        global $connection;

        $time = date('d M Y | h:i A');
        $action = $userFirstName . ' ' . $userLastName . " modified the record of " . $soloParentFullName . " (" . $soloParentControlNumber . ").";
        $user = $username;

        $sql = "INSERT INTO `audittrail` (`TrailID`, `Time`, `Action`, `Details`, `User`) VALUES (NULL, '$time', 'Modification of Solo Parent Record', '$action', '$user')";

        $result = mysqli_query($connection, $sql);
    }

    function addMarkSoloParentIneligibleAudit($userFirstName, $userLastName, $soloParentFullName, $soloParentControlNumber, $username) {
        global $connection;

        $time = date('d M Y | h:i A');
        $action = $userFirstName . ' ' . $userLastName . " marked " . $soloParentFullName . " (" . $soloParentControlNumber . ") as ineligible to solo parent membership.";
        $user = $username;

        $sql = "INSERT INTO `audittrail` (`TrailID`, `Time`, `Action`, `Details`, `User`) VALUES (NULL, '$time', 'Modification of Solo Parent Membership Status', '$action', '$user')";

        $result = mysqli_query($connection, $sql);
    }
    
    function addRenewSoloParentAudit($userFirstName, $userLastName, $soloParentFullName, $soloParentControlNumber, $username) {
        global $connection;

        $time = date('d M Y | h:i A');
        $action = $userFirstName . ' ' . $userLastName . " renewed " . $soloParentFullName . " (" . $soloParentControlNumber . ") solo parent membership". ".";
        $user = $username;

        $sql = "INSERT INTO `audittrail` (`TrailID`, `Time`, `Action`, `Details`, `User`) VALUES (NULL, '$time', 'Renewal of Solo Parent Membership', '$action', '$user')";

        $result = mysqli_query($connection, $sql);
    }

    function addUploadMaterialAudit($userFirstName, $userLastName, $materialName, $username) {
        global $connection;

        $time = date('d M Y | h:i A');
        $action = $userFirstName . ' ' . $userLastName . " (" . $username . ")" . " uploaded " . $materialName . " to the system.";
        $user = $username;

        $sql = "INSERT INTO `audittrail` (`TrailID`, `Time`, `Action`, `Details`, `User`) VALUES (NULL, '$time', 'Upload of Informative Material', '$action', '$user')";

        $result = mysqli_query($connection, $sql);
    }

    function addDeleteMaterialAudit($userFirstName, $userLastName, $materialName, $username) {
        global $connection;

        $time = date('d M Y | h:i A');
        $action = $userFirstName . ' ' . $userLastName . " (" . $username . ")" . " deleted " . $materialName . " from the system.";
        $user = $username;

        $sql = "INSERT INTO `audittrail` (`TrailID`, `Time`, `Action`, `Details`, `User`) VALUES (NULL, '$time', 'Deletion of Informative Material', '$action', '$user')";
